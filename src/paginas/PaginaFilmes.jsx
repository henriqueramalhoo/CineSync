...
function ConteudoPaginaFilmes() {
  ...
  useEffect(() => {
    ...
    const buscar = async () => {
      ...
      const params = {
        ...
        primary_release_year: filtros.year,
        with_origin_country: filtros.country,
        with_original_language: filtros.language,
        sort_by: filtros.sort_by,
        page: paginaAtual, // Adiciona a página atual
      };
      Object.keys(params).forEach(key => !params[key] && delete params[key]);
      
      const dados = await descobrirFilmes(params);
      setResultadosDescoberta(dados.results);
      setTotalPaginas(dados.total_pages > 500 ? 500 : dados.total_pages); // API do TMDB limita a 500 páginas
      setCarregando(false);
    };
  ...
  const handleFiltroChange = (nome, valor) => {
    ...
    setPaginaAtual(1); // Reseta a página ao mudar de filtro
  };
  ...
  return (
    <div className="container mx-auto p-4 pt-28">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100 mb-4">Filmes</h1>
        <BarraPesquisa />
      </div>
      <Filtros
      ...
      {aCarregar ? <Spinner /> : (
        <>
          <GrelhaItens itens={mostrarResultados} tipo="movie" />
          <Paginacao 
...
function PaginaFilmes() {
  return (
    <PesquisaProvider procurarFn={procurarFilmes}>
      <ConteudoPaginaFilmes />
    </PesquisaProvider>
  );
}
